...
def _render_results(
    request: Request, result: analysis.AnalysisResult, job_id: str
) -> HTMLResponse:
...
async def analyze_endpoint(request: Request, file: UploadFile = File(...)):
    ...
    job.audio_bytes = data
    job.audio_content_type = file.content_type or "audio/wav"
    JOBS[job.job_id] = job
...
async def progress_endpoint(request: Request, job_id: str):
    ...
    if job.done:
        ...
        if job.result is not None:
            return _render_results(request, job.result, job.job_id)
    return _render_progress(request, job)
...
async def audio_endpoint(job_id: str):
    job = JOBS.get(job_id)
    if job is None or job.audio_bytes is None:
        raise HTTPException(status_code=404, detail="audio not available")
    return Response(
        content=job.audio_bytes,
        media_type=job.audio_content_type,
